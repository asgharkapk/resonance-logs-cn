<script lang="ts">
  ...
  // --- Challenge watch (forbidden damage ids) ---
  ...
  function removeForbiddenId(id: number) {
    setForbiddenDamageIds(forbiddenDamageIds.filter((existing) => existing !== id));
  }
...
<Tabs.Content value={SETTINGS_CATEGORY}>
  <div class="space-y-3">
    ...
    >
      <button
        type="button"
        class="w-full flex items-center justify-between px-4 py-3 hover:bg-muted/30 transition-colors cursor-pointer"
