<script lang="ts">
	...
	const effectivePlaceholder = $derived(
		placeholder ?? t('components.playerAutocomplete.placeholder')
	);
...
<div class="player-autocomplete-container relative">
	...
	{#if isLoading}
		<div class="absolute z-10 w-full mt-1 bg-neutral-800 border border-neutral-700 rounded shadow-lg px-3 py-2">
			<div class="text-neutral-500 text-sm">{t('components.playerAutocomplete.loading')}</div>
		</div>
	...
	{:else if value.trim().length >= 1 && !isLoading && filteredPlayerNames.length === 0}
		<div class="absolute z-10 w-full mt-1 bg-neutral-800 border border-neutral-700 rounded shadow-lg px-3 py-2">
			<div class="text-neutral-500 text-sm">{t('components.playerAutocomplete.noPlayers')}</div>
		</div>
	{/if}
</div>
