...
{#if filters.length > 0}
	<div class="flex flex-wrap gap-2 items-center">
		<span class="text-sm text-neutral-400">{t('components.filterChips.label')}</span>
		{#each filters as bossName}
			<div
				...
				aria-label={t('components.filterChips.removeAria', { name: bossName })}
			>
				<span>{bossName}</span>
				<button
		...
		<button
			...
			aria-label={t('components.filterChips.clearAllAria')}
		>
			{t('components.filterChips.clearAll')}
		</button>
	</div>
{/if}
